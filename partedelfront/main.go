...
var (
	...
	Values  [1000]string
	Times2  [1000]string
	Values2 [1000]string //para cpu0
	Values3 [1000]string //para cpu1
	Values4 [1000]string //para cpu2
	Values5 [1000]string //para cpu3
)
...
func obtenerram(w http.ResponseWriter, r *http.Request) {
	...
	memolibre := strings.Replace((listainfo[1])[10:24], " ", "", -1)
	...
	ramlibrereal, err2 := strconv.Atoi(memolibre)
	if err1 == nil && err2 == nil {
		ramtotalmb := ramtotal / 1024
		ramlibremb := ramlibre / 1024
		libremb := ramlibrereal / 1024
		...
		val2 := strconv.Itoa(porcentajeram)
		fmt.Println("EL LIBRE ES ", val2)
		...
		*/
		respuestamem := Strumemoria{Total: ramtotalmb, Libre: libremb, Disponible: ramlibremb, Porcentaje: val2, Tiempos: strings.Join(Times[:], ","), Valores: strings.Join(Values[:], ","), Index: index}
		...
		//	fmt.Println("EL indice es ", conver)
...
func obtenercpu(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil {
		...
		//	fmt.Printf("LOS PORCENTAJES SON ", percentage)
		//para cpu 0
		porcent := strconv.FormatFloat(percentage[0], 'f', 5, 64)
		Values2[index2] = strconv.FormatFloat(percentage[0], 'f', 5, 64)
		//para cpu1
		Values3[index2] = strconv.FormatFloat(percentage[0], 'f', 5, 64)
		//para cpu2
		Values4[index2] = strconv.FormatFloat(percentage[0], 'f', 5, 64)
		//para cpu3
		Values4[index2] = strconv.FormatFloat(percentage[0], 'f', 5, 64)
		...
		var respuestamem = CPU2{Cores: cpuStat[0].Cores, Vendor: cpuStat[0].VendorID, Family: cpuStat[0].Family, Model: cpuStat[0].ModelName, Speed: strconv.FormatFloat(cpuStat[0].Mhz, 'f', 2, 64), Read: read, Tiempos: strings.Join(Times2[:], ","), Valores: strings.Join(Values2[:], ","), Index: index2, Porcentaje: porcent}
...
func reply(w http.ResponseWriter, r *http.Request) {
	...
	if t.Hat != "" {
		fmt.Println("Eliminando proceso con id: ", t.Hat)
		app := "kill"
...
func main() {
	//obtener ram
	http.HandleFunc("/obtenerram", obtenerram)
